...
export class PerfilService extends RestBaseService {
  ...
  buscarImagen(id: string): Promise<Image> {
    return this.http
      ...
      .toPromise()
      .then(response => {
        console.log("response de buscarImagen: " + response);
  ...
  buscarPerfilById(id: number): Promise<Perfil> {
    return this.http
      .get(PerfilService.serverUrl + this.perfilUrl + "/" + id, this.getRestHeader())
      .toPromise()
      .then(response => {
        return response.json() as Perfil;
      })
